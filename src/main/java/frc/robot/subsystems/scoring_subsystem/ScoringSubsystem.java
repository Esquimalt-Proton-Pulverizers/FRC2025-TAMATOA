...
public class ScoringSubsystem extends SubsystemBase{
    ...
    public enum Position {
        ...
        CORAL_GROUND_INTAKE     (-110.0,     +90.0,  +4.5),
        ALGAE_GROUND_INTAKE     (-115.0,    -90.0,  +5.2), // not yet tested
        CORAL_STATION_INTAKE    (+360.0,      +0.0,   +360.0), // not yet determined
        ALGAE_LOLLIPOP_INTAKE   (-115.0,    -90.0,  +11.25), // not yet tested
        HOME_FOR_CLIMB          (-5.0,      +0.0,   +1.0),
        SET_CORAL_POSITION_LEFT (+360.0,      +360.0,   +360.0), // not yet determined
        SET_CORAL_POSITION_RIGHT(+360.0,      +0.0,   +360.0), // not yet determined
        SCORE_L1               (-26.0,     +90.0,  +5.0), // Works
        SCORE_L2_MID                (-30.27,     +0.0,   +12.5), // Works
        SCORE_L3_MID               (-30.27,     +0.0,   +28.5), // Works 
        SCORE_L4_MID          (-51.5,     +0.0,   +58.0), // Works
        SCORE_L2_LEFT                (-30.27,     +0.0,   +20), // 
        SCORE_L3_LEFT              (-30.27,     +0.0,   +28.5), //  
        SCORE_L4_LEFT          (-51.5,     +0.0,   +58.0), // 
        SCORE_L2_RIGHT                (-30.27,     +0.0,   +30), // 
        SCORE_L3_RIGHT               (-30.27,     +0.0,   +28.5), //  
        SCORE_L4_RIGHT          (-51.5,     +0.0,   +58.0), // 

        SCORE_NET               (+360.0,      +0.0,   +360.0), // not yet determined
    ...
    public Command moveElevator(double targetPosition) {
		return new ElevatorToPosCommand(targetPosition, elevatorSubsystem);
	}

    
    public void initialize() {
        differentialSubsystem.initialize(0,0); //TODO make a class variable for this, or pull from start position
    }
    ...
    private Command returnDWECommand(double[] targetVals, ElevatorSubsystem elevatorSubsystem, DifferentialSubsystem elbowSubsystem) {
        System.out.println("DWE run");
        return new DifferentialElevationRotationCommand(targetVals[0], startDiffWristAngle, elbowSubsystem)
    ...
    private Command returnWDECommand(double[] targetVals, ElevatorSubsystem elevatorSubsystem, DifferentialSubsystem elbowSubsystem) {
        ...
        return new DifferentialElevationRotationCommand(startDiffElevationAngle, targetVals[1], elbowSubsystem)
        .andThen(new DifferentialElevationRotationCommand(targetVals[0], targetVals[1], elbowSubsystem))
        .andThen(new ElevatorToPosCommand(targetVals[2], elevatorSubsystem));
    }
    ...
    private Command returnEWDCommand(double[] targetVals, ElevatorSubsystem elevatorSubsystem, DifferentialSubsystem elbowSubsystem) {
        ...
        .andThen(new DifferentialElevationRotationCommand(startDiffElevationAngle, targetVals[1], elbowSubsystem))
        .andThen(new DifferentialElevationRotationCommand(targetVals[0], targetVals[1], elbowSubsystem));
    }
    ...
    private Command returnEDWCommand(double[] targetVals, ElevatorSubsystem elevatorSubsystem, DifferentialSubsystem elbowSubsystem) {
        ...
        return new ElevatorToPosCommand(targetVals[2], elevatorSubsystem)
        .andThen(new DifferentialElevationRotationCommand(targetVals[0], startDiffWristAngle, elbowSubsystem))
        .andThen(new DifferentialElevationRotationCommand(targetVals[0], targetVals[1], elbowSubsystem));
    }
